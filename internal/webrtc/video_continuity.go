...
func (v *videoContinuity) translate(pkt *rtp.Packet, now time.Time) (*rtp.Packet, bool) {
	...
	switching := v.seen && v.ssrc != pkt.SSRC
	if switching && (err != nil || descriptor.S != 1 || descriptor.PID != 0 || len(data) < 10 || data[0]&1 != 0 || data[3] != 0x9d || data[4] != 1 || data[5] != 0x2a) {
		return nil, false
	}
	first := !v.seen
	if switching {
	...
	if err == nil && (descriptor.I != 0 || v.tl0Offset != 0 || v.keyOffset != 0) {
		...
		out.Payload = append(header, data...)
	}
	if first || switching || int16(out.SequenceNumber-v.sequence) > 0 {
		...
		v.seen, v.ssrc, v.sequence = true, pkt.SSRC, out.SequenceNumber
