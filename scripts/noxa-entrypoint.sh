...
secret_env_load NOXA_DATABASE_URL optional
secret_env_load NOXA_REDIS_PASSWORD optional
secret_env_load NOXA_TURN_SECRET optional
...
if [ -z "$NOXA_DATABASE_URL" ] && [ -n "${POSTGRES_HOST:-}" ]; then
	secret_env_load POSTGRES_PASSWORD required
	postgres_host=${POSTGRES_HOST:-postgres}
	postgres_port=${POSTGRES_PORT:-5432}
	postgres_user=${POSTGRES_USER:-noxa}
	postgres_database=${POSTGRES_DB:-noxa}
	postgres_sslmode=${POSTGRES_SSLMODE:-disable}
	NOXA_DATABASE_URL="postgres://$(url_escape "$postgres_user"):$(url_escape "$POSTGRES_PASSWORD")@${postgres_host}:${postgres_port}/${postgres_database}?sslmode=${postgres_sslmode}"
	export NOXA_DATABASE_URL
fi
