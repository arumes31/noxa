...
func (e *Chromium) Focus() {
	...
	// Native modal pickers disable their owner. Forwarding owner focus into
	// WebView2 during that nested message loop can return E_INVALIDARG.
	if e.hwnd != 0 {
		enabled, _, _ := w32.User32IsWindowEnabled.Call(e.hwnd)
		if enabled == 0 {
			return
		}
	}
	err := e.controller.MoveFocus(COREWEBVIEW2_MOVE_FOCUS_REASON_PROGRAMMATIC)
	if errors.Is(err, windows.Errno(windows.E_INVALIDARG)) {
		log.Printf("[WebView2] focus deferred: %v", err)
		return
	}
	if err != nil {
		e.errorCallback(err)
